...
module.exports = {
  post: async (req, res) => {
    ...
    if (!liker) {   // 유저가 좋아요 누른 게시물이 아니라면 좋아요 +1 실행 후 DB에 create.
      ...
      const erc20Contract = new web3.eth.Contract(erc20Abi, ERC20_ADDR, { from: EXCHANGE_ADDR });   // contract와 연결
      const tokenFaucet = await erc20Contract.methods.transfer(address, '1000000000000000000').send({ gasLimit: 200000 });    // 토큰을 지급하는 로직.
